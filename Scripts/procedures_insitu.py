    ...
    def get_icme_list(self,filename,start_time,end_time,spacecraft):
        ...
        if(spacecraft == 'earth'):
            ...
            data = pd.read_csv(filename,comment='#',
                   header=0,sep='\s+',usecols=[0,1,3],parse_dates=[0,1])
            data.columns=['start_time','end_time','STEREO']
            data['start_time']=[x.tz_convert(None) for x in data['start_time']]
            data['end_time']=[x.tz_convert(None) for x in data['end_time']]
            #decide sta or stb
            if(spacecraft == 'sta'):
                data_icme=data[data['STEREO']==1]
            elif(spacecraft == 'stb'):
                data_icme=data[data['STEREO']==-1]
